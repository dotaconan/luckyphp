        ...
        DDLClass.prototype = {
            ...
            findIndexByValue: function (list, value) {
                ...
                for (var i = 0; i < list.length; i++) {
                    var currentIndexValue = list[i][1];
            ...
            findSelectedIndex: function (list) {
                var index = 0;
                for (var i = 0; i < list.length; i++) {
                    var currentIndexValue = list[i][2];
                    if (currentIndexValue === true) {
                        return i;
                    }
                }
                return index;
            },
            setSelectItemByValue: function (value) {
                var self = this;
                //根据传入的值获取索引
                var selectedIndexByValue = self.findIndexByValue(self.configs.options, value);
                //获取当前选中索引
                var findSelectedIndex= self.findSelectedIndex(self.configs.options);
                self.select.val(value);
                self.select.trigger('change');
                self.hide().listEle.trigger('hide-list');
            },
